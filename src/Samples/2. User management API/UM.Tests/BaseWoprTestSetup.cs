...
public class BaseUMTestSetup : TestSetup
{
    ...
    public override void BeforeStart()
    {
        ...
        PasswordPerUser.Add(IExecutionScope.ROOTUSER, Guid.NewGuid().ToString());
    }
    ...
    public override async Task Start()
    {
        ...
        if (!DisablePasswordChange)
        {
            ...
            RootTestInstance = await NewUserTestInstance(IExecutionScope.ROOTUSER, AuthenticationHostName);
    ...
    public async Task UpdateRootPassword()
    {
        ...
        else
        {
            using (var userTestInstance = await NewUserTestInstance(string.Empty, AuthenticationHostName))
            {
                ...
                {
                    needRootPasswordUpdate = await client.UMFacade_ServerStatus_NeedTechUserPasswordUpdate(IExecutionScope.ROOTUSER);
                }
            }
        }
        ...
        if (needRootPasswordUpdate)
        {
            using (var userTestInstance = await NewUserTestInstance(string.Empty, AuthenticationHostName))
            {
                using (var client = userTestInstance.PickClient(AuthenticationHostName))
                {
                    await client.UMFacade_Authentication_SetInitialPassword(IExecutionScope.ROOTUSER, PasswordPerUser[IExecutionScope.ROOTUSER]);
                }
            }
        }
    }
    ...
    public async Task<string> CreateUserWithRoot(string organizationId, string name, string roleName = null, List<ApplicationRightDTO> rights = null)
    {
        using (var userTestInstance = await NewUserTestInstance(IExecutionScope.ROOTUSER, AuthenticationHostName))
        {
            var client = userTestInstance.PickClient(AuthenticationHostName);
