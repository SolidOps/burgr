...
public partial class UserCreationService
{
    ...
    {
        var organizationId = Guid.Parse(user_creation_info.OrganizationId);

        var user = User.Create(user_creation_info.UserEmail, null, false);
        if (user_creation_info.RoleName == null && user_creation_info.Rights != null && user_creation_info.Rights.Any())
        {
            user.ApplicationRights = user_creation_info.Rights;
        }
