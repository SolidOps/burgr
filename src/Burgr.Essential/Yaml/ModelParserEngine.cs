    ...
    public void BeforeParsing()
    {
        if (!IsInitialized)
        {
            ...
            foreach (string file in specFiles)
            {
                ...
                var version = lines.First().Replace("joshua-model-version: ", string.Empty);
        ...
        foreach (KeyValuePair<string, FullModelDescription> kvp in modelsRepository.modelDescriptions)
        {
            ...
            foreach (ModelDescriptor service in description.ModelDescriptors.Where(m => m.DescriptorType == ServiceGenerator.Name))
            {
                string[] descriptors = new string[] {
                    VoidMethodGenerator.Name,
                    SimpleMethodGenerator.Name,
                    ModelMethodGenerator.Name,
                    ModelListMethodGenerator.Name,
                    IdentityMethodGenerator.Name
                };
